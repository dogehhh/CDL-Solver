...
# WITHOUT WARRANTIxES OR CONDITIONS OF ANY KIND, either express or implied.
...
class RayPPOTrainer:
    ...
    def _validate(self, only_val=False) -> dict[str, Any]:
        reward_tensor_lst = []
        # Lists to collect samples for the table
        sample_inputs, sample_outputs, sample_labels, sample_scores, sample_proIDs, sample_questions = [], [], [], [], [], []
        ...
        for batch_dict in self.val_dataloader:
            ...
            sample_proIDs.extend(test_batch.non_tensor_batch["proID"].tolist())
            sample_questions.extend(input_texts)
        ...
        if only_val:
            print('only val, generate log')
            root = '/workspace/user_code/EasyR1/checkpoints/easy_r1/qwen3_vl_8b_formalgeo7kv2_CoT_consCDL_0930_f1score/'
            eval_file = root + 'qwen3_vl_8b_unigeo_train_cal_angle_gen_judge.json'
            output = {}
            output["accuracy"] = 0.0
            results = []
            for sample_output, sample_label, sample_proID, sample_question in tqdm(zip(sample_outputs, sample_labels, sample_proIDs, sample_questions)):
                result = {"model_output": sample_output, "ground_truth": sample_label}
                result["question"] = {"image_path": str(sample_proID) + '.png', "question": sample_question}
    ...
    def fit(self):
        ...
        if self.val_reward_fn is not None and self.config.trainer.val_before_train:
            val_metrics = self._validate(self.config.trainer.val_only)
