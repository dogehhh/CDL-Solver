...
def format_reward(response: str) -> float:
    pattern = r"<think>([\s\S]*?)<\/think>\s*<CDL>([\s\S]*?)<\/CDL>\s*<solution>.*</solution>.*\\boxed\{.*\}.*"
...
def solve_acc_reward(response: str, ground_truth: str) -> float:
...
def compute_score(reward_inputs: List[Dict[str, Any]], format_weight: float = 0.5) -> List[Dict[str, float]]:
    ...
    for reward_input in reward_inputs:
        ...
        textCDL_recall_score, textCDL_precision_score = textCDL_acc_reward(reward_input["response"], reward_input["cdl"])
        consCDL_recall_score, consCDL_precision_score = consCDL_acc_reward(reward_input["response"], reward_input["cdl"])
        imgCDL_recall_score, imgCDL_precision_score = imgCDL_acc_reward(reward_input["response"], reward_input["cdl"])
        acc_score = solve_acc_reward(reward_input["response"], reward_input["ground_truth"])
        overall_score = 0.1 * format_score + (1 - format_weight) * (consCDL_recall_score + consCDL_precision_score) / 2 + format_weight * (imgCDL_precision_score + imgCDL_recall_score + textCDL_precision_score + textCDL_recall_score) / 4
        overall_score = 0.1 * overall_score + 0.9 * acc_score
        scores.append(
            {
